...
import com.sturdy.myapp.domain.ApplicationUser;
...
public class ApplicationUserRepositoryWithBagRelationshipsImpl implements ApplicationUserRepositoryWithBagRelationships {
    ...
    public Optional<ApplicationUser> fetchBagRelationships(Optional<ApplicationUser> applicationUser) {
        return applicationUser.map(this::fetchEvents);
    }
    ...
    public List<ApplicationUser> fetchBagRelationships(List<ApplicationUser> applicationUsers) {
        return Optional.of(applicationUsers).map(this::fetchEvents).orElse(Collections.emptyList());
    }

    ApplicationUser fetchEvents(ApplicationUser result) {
        return entityManager
            .createQuery(
                "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.events where applicationUser is :applicationUser",
    ...
    List<ApplicationUser> fetchEvents(List<ApplicationUser> applicationUsers) {
        ...
        List<ApplicationUser> result = entityManager
            .createQuery(
                "select distinct applicationUser from ApplicationUser applicationUser left join fetch applicationUser.events where applicationUser in :applicationUsers",
                ApplicationUser.class
            )
