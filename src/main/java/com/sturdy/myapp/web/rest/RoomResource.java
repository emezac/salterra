...
/**
 * REST controller for managing {@link com.sturdy.myapp.domain.Room}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Room> partialUpdateRoom(@PathVariable(value = "id", required = false) final Long id, @RequestBody Room room)
        ...
        Optional<Room> result = roomRepository
            ...
            .map(existingRoom -> {
                ...
                if (room.getHaveGatewayDoor() != null) {
                    existingRoom.setHaveGatewayDoor(room.getHaveGatewayDoor());
                }
                if (room.getGwyDoor1() != null) {
                    existingRoom.setGwyDoor1(room.getGwyDoor1());
                }
                if (room.getGwyDoor2() != null) {
                    existingRoom.setGwyDoor2(room.getGwyDoor2());
                }
                if (room.getGwyDoor3() != null) {
                    existingRoom.setGwyDoor3(room.getGwyDoor3());
                }
                if (room.getGwyDoor4() != null) {
                    existingRoom.setGwyDoor4(room.getGwyDoor4());
                }
                if (room.getGwyDoor5() != null) {
                    existingRoom.setGwyDoor5(room.getGwyDoor5());
                }
                if (room.getGwyDoor6() != null) {
                    existingRoom.setGwyDoor6(room.getGwyDoor6());
                }
