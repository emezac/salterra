...
/**
 * REST controller for managing {@link com.sturdy.myapp.domain.Card}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CardResource {
    ...
    public ResponseEntity<Card> partialUpdateCard(@PathVariable(value = "id", required = false) final Long id, @RequestBody Card card)
        ...
        Optional<Card> result = cardRepository
            .findById(card.getId())
            .map(existingCard -> {
                if (card.getThumbnaiImage() != null) {
                    existingCard.setThumbnaiImage(card.getThumbnaiImage());
                }
                if (card.getThumbnaiImageContentType() != null) {
                    existingCard.setThumbnaiImageContentType(card.getThumbnaiImageContentType());
                }
                if (card.getHighresImage() != null) {
                    existingCard.setHighresImage(card.getHighresImage());
                }
                if (card.getHighresImageContentType() != null) {
                    existingCard.setHighresImageContentType(card.getHighresImageContentType());
                }
                if (card.getCreatedAt() != null) {
                    existingCard.setCreatedAt(card.getCreatedAt());
                }
                if (card.getUpdatedAt() != null) {
                    existingCard.setUpdatedAt(card.getUpdatedAt());
                }
                if (card.getDeletedAt() != null) {
                    existingCard.setDeletedAt(card.getDeletedAt());
                }
