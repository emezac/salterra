...
/**
 * REST controller for managing {@link com.sturdy.myapp.domain.Profile}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ProfileResource {
    ...
    public ResponseEntity<Profile> partialUpdateProfile(
        ...
        Optional<Profile> result = profileRepository
            ...
            .map(existingProfile -> {
                ...
                if (profile.getSuspended() != null) {
                    existingProfile.setSuspended(profile.getSuspended());
                }
                if (profile.getSuspendedCount() != null) {
                    existingProfile.setSuspendedCount(profile.getSuspendedCount());
                }
                if (profile.getBanned() != null) {
                    existingProfile.setBanned(profile.getBanned());
                }
                if (profile.getAclSetup() != null) {
                    existingProfile.setAclSetup(profile.getAclSetup());
                }
                if (profile.getCreatedAt() != null) {
                    existingProfile.setCreatedAt(profile.getCreatedAt());
                }
                if (profile.getUpdatedAt() != null) {
                    existingProfile.setUpdatedAt(profile.getUpdatedAt());
                }
                if (profile.getDeletedAt() != null) {
                    existingProfile.setDeletedAt(profile.getDeletedAt());
                }
     ...
     */
    @GetMapping("/profiles")
    public List<Profile> getAllProfiles(@RequestParam(required = false) String filter) {
        if ("profilegamestatus-is-null".equals(filter)) {
            log.debug("REST request to get all Profiles where profileGameStatus is null");
            return StreamSupport
                .stream(profileRepository.findAll().spliterator(), false)
                .filter(profile -> profile.getProfileGameStatus() == null)
