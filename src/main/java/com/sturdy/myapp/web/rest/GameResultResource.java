...
/**
 * REST controller for managing {@link com.sturdy.myapp.domain.GameResult}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class GameResultResource {
    ...
    public ResponseEntity<GameResult> updateGameResult(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, gameResult.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<GameResult> partialUpdateGameResult(
        ...
        Optional<GameResult> result = gameResultRepository
            ...
            .map(existingGameResult -> {
                if (gameResult.getWon() != null) {
                    existingGameResult.setWon(gameResult.getWon());
                }
                if (gameResult.getLost() != null) {
                    existingGameResult.setLost(gameResult.getLost());
                }
                if (gameResult.getCreatedAt() != null) {
                    existingGameResult.setCreatedAt(gameResult.getCreatedAt());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, gameResult.getId().toString())
        );
    }
