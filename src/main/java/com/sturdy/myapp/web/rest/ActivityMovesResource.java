...
public class ActivityMovesResource {
    ...
    public ResponseEntity<ActivityMoves> partialUpdateActivityMoves(
        ...
        Optional<ActivityMoves> result = activityMovesRepository
            ...
            .map(existingActivityMoves -> {
                if (activityMoves.getMoveDate() != null) {
                    existingActivityMoves.setMoveDate(activityMoves.getMoveDate());
                }
    ...
    public List<ActivityMoves> getAllActivityMoves(@RequestParam(required = false) String filter) {
        if ("move-is-null".equals(filter)) {
            log.debug("REST request to get all ActivityMovess where move is null");
            return StreamSupport
                .stream(activityMovesRepository.findAll().spliterator(), false)
                .filter(activityMoves -> activityMoves.getMove() == null)
    ...
    public ResponseEntity<ActivityMoves> getActivityMoves(@PathVariable Long id) {
        ...
        Optional<ActivityMoves> activityMoves = activityMovesRepository.findById(id);
