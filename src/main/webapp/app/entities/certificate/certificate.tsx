            ...
            <tbody>
              {certificateList.map((certificate, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{certificate.course ? <Link to={`/course/${certificate.course.id}`}>{certificate.course.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/certificate/${certificate.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`/certificate/${certificate.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`/certificate/${certificate.id}/delete`} color="danger" size="sm" data-cy="entityDeleteButton">
