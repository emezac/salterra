...
import { getEntities as getPrizePools } from 'app/entities/prize-pool/prize-pool.reducer';
...
export const PrizeUpdate = () => {
  ...
  const prizePools = useAppSelector(state => state.prizePool.entities);
  ...
  const saveEntity = values => {
    const entity = {
      ...
      prizePool: prizePools.find(it => it.id.toString() === values.prizePool.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          room: prizeEntity?.room?.id,
          prizePool: prizeEntity?.prizePool?.id,
        };
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField label="Options" id="prize-options" name="options" data-cy="options" type="text" />
              ...
              <ValidatedField id="prize-room" name="room" data-cy="room" label="Room" type="select">
                <option value="" key="0" />
                {rooms
              ...
              <ValidatedField id="prize-prizePool" name="prizePool" data-cy="prizePool" label="Prize Pool" type="select">
                <option value="" key="0" />
                {prizePools
                  ? prizePools.map(otherEntity => (
                      <option value={otherEntity.id} key={otherEntity.id}>
                        {otherEntity.id}
                      </option>
                    ))
