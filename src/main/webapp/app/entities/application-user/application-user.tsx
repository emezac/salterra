            ...
            <tbody>
              {applicationUserList.map((applicationUser, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{applicationUser.role}</td>
                  <td>{applicationUser.polygonAddress}</td>
                  <td>{applicationUser.publicUrl}</td>
                  <td>
                    {applicationUser.events
                      ? applicationUser.events.map((val, j) => (
                          <span key={j}>
                            <Link to={`/event/${val.id}`}>{val.id}</Link>
                            {j === applicationUser.events.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
